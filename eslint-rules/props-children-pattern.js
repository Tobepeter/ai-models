	...
	create(context) {
		...
		function findChildrenProperty(typeNode) {
			...
			if (typeNode.type === 'TSTypeLiteral') {
				return typeNode.members.find((member) => member.type === 'TSPropertySignature' && member.key && member.key.name === 'children')
			}

			// 处理接口体：interface Props { children: ReactNode }
			if (typeNode.type === 'TSInterfaceBody') {
				return typeNode.body.find((member) => member.type === 'TSPropertySignature' && member.key && member.key.name === 'children')
			}
