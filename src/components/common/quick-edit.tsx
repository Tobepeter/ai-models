...
export const QuickEdit = (props: QuickEditProps) => {
    ...
    return (
      <div className={cn('relative', containerClassName)}>
        <Input
          ...
          className={cn(
            'box-border py-1',
            displayLimit && maxLength > 0 ? 'px-2 pr-12' : 'px-2',
            className
          )}
          style={{ height: '28px' }}
          {...inputProps}
  ...
  return (
    <div 
      className={cn(
        'flex items-center',
        !showEditIcon && 'hover:bg-muted/50 rounded-md p-2 -m-2 transition-colors',
        containerClassName
      )}
    >
      ...
      <div 
        className={cn(
          'flex-1 min-w-0 text-ellipsis overflow-hidden flex items-center',
          !showEditIcon && 'cursor-pointer',
          textClassName
        )}
        style={{ height: '28px' }}
