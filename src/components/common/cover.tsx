import { ComputedTheme, useAppStore } from '@/store/store'
import { cn } from '@/lib/utils'
import { useState } from 'react'
import { EventType, useEvent } from '@/utils/event-bus'
...
export const Cover = () => {
	...
	const [computedTheme, setComputedTheme] = useState<ComputedTheme>(useAppStore().getComputedTheme())

	useEvent(EventType.ThemeUpdate, (theme) => {
		setComputedTheme(theme)
	})

	// light 图片默认有点blur的感觉了，所以不需要额外blur了
	const imgCls = cn('w-full h-full object-cover', computedTheme === 'dark' ? 'blur-sm' : '')
	return (
		<div className="absolute inset-0">
			<img src={computedTheme === 'dark' ? darkImg : lightImg} alt="cover" className={imgCls} />
		</div>
	)
}
