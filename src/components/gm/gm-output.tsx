		...
		<div className="col-span-3 flex flex-col" data-slot="gmoutput">
			...
			<div className="flex-1 border rounded bg-muted/30 overflow-hidden">
				<ScrollArea className="h-[75vh]">
					<div className="p-3">
						{logs.length === 0 ? (
							<div className="text-sm text-muted-foreground text-center py-8">暂无运行进程</div>
						) : (
							logs.map((proc) => (
								<div key={proc.processId} className="mb-4 border rounded bg-background overflow-hidden">
									<div className="flex justify-between items-center p-3 bg-muted/50 border-b">
										...
										<div className="flex gap-2">
											<Badge variant={proc.status === 'running' ? 'default' : proc.status === 'finished' ? 'secondary' : 'destructive'}>{proc.status}</Badge>
											{proc.status === 'running' && (
												<Button size="sm" variant="destructive" onClick={() => onKillProc(proc.processId)} className="h-6 px-2 text-xs">
													终止
												</Button>
											)}
										</div>
