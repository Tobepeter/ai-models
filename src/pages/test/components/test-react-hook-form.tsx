...
const TestReactHookForm = () => {
  ...
  const handleHobbyChange = (hobby: string, checked: boolean) => {
    ...
    if (checked) {
      ...
      basicForm.setValue('hobbies', currentHobbies.filter(h => h !== hobby))
    }
  }
  ...
  const handleSkillChange = (skill: string, checked: boolean) => {
    ...
    if (checked) {
      ...
      advancedForm.setValue('skills', currentSkills.filter(s => s !== skill))
    }
  }
            ...
            <CardContent>
              <form onSubmit={basicForm.handleSubmit(onBasicSubmit)} className="space-y-6">
                ...
                <div className="space-y-2">
                  <Label>国家/地区</Label>
                  <Select
                    value={basicForm.watch('country')}
                    onValueChange={(value) => basicForm.setValue('country', value)}
                  >
                ...
                <div className="space-y-3">
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(value: string) => basicForm.setValue('gender', value)}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {['阅读', '音乐', '运动', '旅行', '编程', '摄影'].map((hobby) => (
                      <div key={hobby} className="flex items-center space-x-2">
                        <Checkbox
                          id={hobby}
                          checked={basicForm.watch('hobbies')?.includes(hobby) || false}
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      <Badge variant={basicForm.formState.isDirty ? 'default' : 'secondary'}>
                        {basicForm.formState.isDirty ? '已修改' : '未修改'}
                      </Badge>
                      <Badge variant={basicForm.formState.isValid ? 'default' : 'destructive'}>
                        {basicForm.formState.isValid ? '验证通过' : '验证失败'}
                      </Badge>
                      <Badge variant={basicForm.formState.isSubmitting ? 'default' : 'secondary'}>
                        {basicForm.formState.isSubmitting ? '提交中' : '未提交'}
                      </Badge>
                    </div>
                  </div>
                  <div className="space-y-2">
              ...
              <form onSubmit={advancedForm.handleSubmit(onAdvancedSubmit)} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    {advancedForm.formState.errors.firstName && (
                      <p className="text-sm text-red-500">{advancedForm.formState.errors.firstName.message}</p>
                    )}
                  </div>
                  <div className="space-y-2">
                    ...
                    {advancedForm.formState.errors.lastName && (
                      <p className="text-sm text-red-500">{advancedForm.formState.errors.lastName.message}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {advancedForm.formState.errors.phone && (
                    <p className="text-sm text-red-500">{advancedForm.formState.errors.phone.message}</p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  />
                  {advancedForm.formState.errors.website && (
                    <p className="text-sm text-red-500">{advancedForm.formState.errors.website.message}</p>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {['JavaScript', 'TypeScript', 'React', 'Vue', 'Node.js', 'Python', 'Java', 'Go', 'Rust'].map((skill) => (
                      <div key={skill} className="flex items-center space-x-2">
                        <Checkbox
                          id={skill}
                          checked={advancedForm.watch('skills')?.includes(skill) || false}
                  ...
                  {(advancedForm.watch('skills')?.length || 0) === 0 && advancedForm.formState.errors.skills && (
                    <p className="text-sm text-red-500">请至少选择一项技能</p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  <Label>工作经验</Label>
                  <Select
                    value={advancedForm.watch('experience')}
                    onValueChange={(value) => advancedForm.setValue('experience', value)}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  />
                  {advancedForm.formState.errors.salary && (
                    <p className="text-sm text-red-500">{advancedForm.formState.errors.salary.message}</p>
                  )}
                </div>
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="newsletter"
                    {...advancedForm.register('newsletter')}
                  />
              ...
              {submittedData ? (
                <div className="space-y-4">
                  <Alert>
                    ...
                    <AlertDescription>
                      {submittedData.type === 'basic' ? '基础表单' : '高级表单'} 提交成功！
                    </AlertDescription>
                  </Alert>
