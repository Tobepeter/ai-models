  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-8">
      ...
      <div>
        ...
        <DragList
          items={simpleControlledItems}
          onItemsChange={setSimpleControlledItems}
          className="max-w-md"
        />
      </div>
      ...
      <div>
        <div className="mb-4">
          ...
          <p className="text-sm text-muted-foreground">
            使用 defaultItems，组件内部管理状态
          </p>
        </div>

        <DragList
          defaultItems={simpleItems}
          onItemsChange={(items) => console.log('非受控模式变化:', items)}
          className="max-w-md"
        />
      </div>
      ...
      <div>
        ...
        <DragList
          items={complexControlledItems}
          onItemsChange={setComplexControlledItems}
      ...
      <div className="mt-8 p-4 bg-muted rounded-lg">
        ...
        <div className="text-sm text-muted-foreground space-y-1">
          <p><strong>受控模式</strong>: 传递 `items` + `onItemsChange`，外部管理状态</p>
          <p><strong>非受控模式</strong>: 传递 `defaultItems`，组件内部管理状态</p>
          <p><strong>简单渲染</strong>: 不传 `renderItem`，使用默认的标题显示</p>
          <p><strong>自定义渲染</strong>: 传递 `renderItem` 函数，完全自定义外观</p>
          <p><strong>样式定制</strong>: 通过 `className` 和 `itemClassName` 控制样式</p>
        </div>
      </div>
    </div>
  )
