								...
								{formatExamples.map((example, index) => (
									<div key={index} className="p-4 border rounded-lg space-y-2">
										<div className="flex items-center justify-between">
											...
											<Button variant="ghost" size="sm" onClick={() => copyToClipboard(numeral(example.value).format(example.format))}>
												复制
											</Button>
										</div>
										<div className="text-sm text-muted-foreground">原值: {example.value}</div>
										<div className="text-sm text-muted-foreground">格式: {example.format}</div>
										<Separator />
										<div className="text-lg font-mono">{numeral(example.value).format(example.format)}</div>
									</div>
								))}
							</div>
						</CardContent>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
								{timeExamples.map((example, index) => (
									<div key={index} className="p-4 border rounded-lg space-y-2">
										...
										<div className="text-sm text-muted-foreground">原值: {example.value} 秒</div>
										<div className="text-sm text-muted-foreground">格式: {example.format}</div>
										<Separator />
										<div className="text-lg font-mono">{numeral(example.value).format(example.format)}</div>
									</div>
						...
						<CardContent className="space-y-4">
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div className="space-y-2">
									...
									<Input id="custom-value" type="text" value={customValue} onChange={(e) => setCustomValue(e.target.value)} placeholder="输入数值，如 1234567.89" />
								</div>
								<div className="space-y-2">
									<Label htmlFor="custom-format">格式</Label>
									<Input id="custom-format" type="text" value={customFormat} onChange={(e) => setCustomFormat(e.target.value)} placeholder="输入格式，如 0,0.00" />
								</div>
							</div>
