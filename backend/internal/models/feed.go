...
// FeedCommentLike 评论点赞模型
type FeedCommentLike struct {
	BaseModel        // 嵌入基础模型，获得uint64 ID + 时间戳
	CommentID uint64 `json:"comment_id" gorm:"not null;uniqueIndex:idx_comment_user_unique" swaggertype:"string"`
	UserID    uint64 `json:"user_id" gorm:"not null;uniqueIndex:idx_comment_user_unique" swaggertype:"string"`

	// 复合唯一索引，防止重复点赞
	// UNIQUE KEY idx_comment_user_unique (comment_id, user_id)
}
